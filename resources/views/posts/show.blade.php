...
@section('content')
    ...
    @if ($post->why_it_matters)
        <div class="mt-4 rounded-2xl border border-[#ff4d73]/15 bg-[#ff4d73]/5 px-5 py-4 text-sm leading-6 text-zinc-600 dark:text-zinc-400">
            <span class="font-semibold text-zinc-800 dark:text-zinc-200">Why it matters:</span> {{ $post->why_it_matters }}
        </div>
    @endif
    ...
    <section id="conversation" class="mt-8 scroll-mt-24">
        ...
        @else
            <a href="{{ route('login') }}" class="mb-5 block rounded-2xl border border-zinc-200 px-5 py-4 text-sm text-zinc-500 hover:text-zinc-950 dark:border-white/8 dark:hover:text-white">Sign in to join the conversation →</a>
        @endauth

        <div class="space-y-3" data-realtime-comments>
            @forelse ($comments as $comment)
                <article id="comment-{{ $comment->id }}" class="loom-card scroll-mt-24 p-5">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            @foreach ($comment->replies as $reply)
                                <div id="comment-{{ $reply->id }}" class="mt-4 scroll-mt-24 border-l border-zinc-200 pl-4 dark:border-white/10">
