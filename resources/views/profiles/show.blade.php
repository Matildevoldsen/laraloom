...
@section('content')
    <div data-realtime-profile>
        <section class="loom-card mb-6 overflow-hidden">
            ...
            <div class="px-5 pb-5 sm:px-7">
                <div class="-mt-10 flex items-end justify-between gap-4">
                    <div class="grid size-20 place-items-center rounded-full border-4 border-[#111319] bg-gradient-to-br from-zinc-700 to-zinc-900 text-2xl font-semibold">{{ str($user->name)->substr(0, 1)->upper() }}</div>
                    <div class="mb-1">
                        @auth
                            @if (auth()->id() === $user->id)
                                <a class="rounded-full border border-white/10 px-4 py-2 text-sm hover:border-white/20" href="{{ route('profiles.edit', $user) }}">Edit profile</a>
                            @else
                                <form method="POST" action="{{ route('profiles.follow', $user) }}">@csrf
                                    <button @class(['loom-button', 'bg-white/10 text-white shadow-none' => $user->is_following])>{{ $user->is_following ? 'Unfollow' : 'Follow' }}</button>
                                </form>
                            @endif
                        @endauth
                    </div>
                ...
                <div class="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-zinc-500">
                    ...
                    @if ($user->website_url)<a class="hover:text-zinc-300" href="{{ $user->website_url }}" rel="me noopener noreferrer" target="_blank">Website ↗</a>@endif
                    <flux:modal.trigger name="profile-followers"><button><b class="font-medium text-zinc-200">{{ $user->followers_count }}</b> followers</button></flux:modal.trigger>
        ...
        <nav class="mb-5 flex gap-1 overflow-x-auto border-b border-white/8" aria-label="Profile content">
            @foreach (['posts' => 'Posts', 'replies' => 'Replies', 'reposts' => 'Reposts', 'likes' => 'Likes', 'projects' => 'Packages'] as $tab => $label)
                <a href="{{ route('profiles.show', [$user, 'tab' => $tab]) }}" @class(['border-b-2 px-4 py-3 text-sm font-medium transition', 'border-[#ff4d73] text-white' => $activeTab === $tab, 'border-transparent text-zinc-500 hover:text-zinc-200' => $activeTab !== $tab])>{{ $label }}</a>
            @endforeach
        </nav>
        ...
        @foreach (['followers' => $followers, 'following' => $following] as $relationship => $people)
            <flux:modal name="profile-{{ $relationship }}" class="md:w-[32rem]">
                <div class="space-y-4"><flux:heading size="lg">{{ ucfirst($relationship) }}</flux:heading><div class="divide-y divide-white/8">@forelse ($people as $person)<a href="{{ route('profiles.show', $person) }}" class="flex items-center gap-3 py-4"><div class="grid size-11 place-items-center rounded-full bg-white/5 font-semibold">{{ str($person->name)->substr(0, 1)->upper() }}</div><div><p class="font-medium text-zinc-100">{{ $person->name }}</p><p class="text-xs text-zinc-500">{{ '@'.$person->username }}</p></div></a>@empty<p class="py-8 text-center text-sm text-zinc-500">Nobody here yet.</p>@endforelse</div></div>
            </flux:modal>
        @endforeach
    </div>
@endsection
